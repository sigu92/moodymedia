...
export const CheckoutSuccess: React.FC = () => {
  ...
  const isMockPayment = searchParams.get('mock') === 'true' || (sessionId && sessionId.startsWith('cs_test_mock_'));
    ...
    const verifyPayment = async () => {
      ...
      try {
        ...
        if (isMockPayment) {
          ...
          if (pendingOrderData) {
            const mockOrderData = JSON.parse(pendingOrderData);
            setVerificationStatus('success');
            setOrderDetails({
              orderId: mockOrderData.orderId || orderId || 'Unknown',
              orderNumber: mockOrderData.orderNumber,
              amount: mockOrderData.amount,
              currency: mockOrderData.currency || 'EUR',
            });
            
            toast({
              title: "Mock Payment Successful!",
              description: "Your test order has been completed using the mock payment system.",
            });
          } else {
            setVerificationStatus('error');
            setErrorMessage('Mock payment data not found. Please try the checkout process again.');
          }
        } else {
          // Handle the real Stripe return and create order
          const result = await handleStripeReturn(sessionId);
          
          if (result.success === true && result.orderData != null && result.orderData.id) {
            setVerificationStatus('success');
            setOrderDetails({
      ...
      <Card className="w-full max-w-lg">
        ...
        <CardContent className="space-y-6">
          <div className="text-center space-y-2">
            ...
            <p className="text-muted-foreground">
              {isMockPayment 
                ? 'Your test order has been completed using the mock payment system.' 
          ...
          {orderDetails.orderId && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <div className="space-y-1 text-sm text-green-700">
                ...
                <p><strong>Payment Method:</strong> {isMockPayment ? 'Mock Payment (Test)' : 'Stripe'}</p>
                <p><strong>Status:</strong> {isMockPayment ? 'Test Completed' : 'Confirmed'}</p>
              </div>
            </div>
          )}
            ...
            {isMockPayment ? (
              <div className="space-y-2 text-sm text-muted-foreground">
                <div className="flex items-start gap-2">
                  ...
                  <p>This was a test order - no real processing will occur</p>
                </div>
                <div className="flex items-start gap-2">
                  <div className="w-2 h-2 bg-orange-500 rounded-full mt-2 flex-shrink-0" />
                  <p>Use this for testing the checkout flow and UI</p>
                </div>
                <div className="flex items-start gap-2">
                  <div className="w-2 h-2 bg-orange-500 rounded-full mt-2 flex-shrink-0" />
                  <p>Switch to real Stripe integration for production</p>
                </div>
              </div>
            ) : (
              <div className="space-y-2 text-sm text-muted-foreground">
