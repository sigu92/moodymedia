    ...
    const loadPaymentDetails = () => {
      ...
      if (sessionId && pendingOrderData) {
        try {
          ...
          if (stripeConfig.isTestMode || !stripeConfig.isConfigured()) {
            setPaymentDetails({
                    ...
                    <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            try {
                              const url = new URL(paymentDetails.stripeReceiptUrl!);
                              const allowedHosts = ['stripe.com', 'buy.stripe.com', 'checkout.stripe.com'];
                              
                              if (!['http:', 'https:'].includes(url.protocol)) {
                                console.error('Invalid URL protocol for receipt:', url.protocol);
                                toast({
                                  title: "Invalid Receipt URL",
                                  description: "Unable to open receipt - invalid URL format.",
                                  variant: "destructive"
                                });
                                return;
                              }
                              ...
                              if (!allowedHosts.some(host => url.hostname === host || url.hostname.endsWith(`.${host}`))) {
                                console.error('Invalid hostname for receipt:', url.hostname);
                                toast({
                                  title: "Invalid Receipt URL",
                                  description: "Unable to open receipt - invalid URL source.",
                                  variant: "destructive"
                                });
                                return;
                              }
                              ...
                              console.error('Error validating receipt URL:', error);
                              toast({
                                title: "Invalid Receipt URL",
                                description: "Unable to open receipt - please contact support.",
                                variant: "destructive"
                              });
                            }
                          }}
