...
export const performSecurityAudit = (): SecurityAuditResult => {
  ...
  const publishableKeyValid = apiKeyManager.validateKey(stripeConfig.getPublishableKey(), 'publishable');
  const secretKeyValid = apiKeyManager.validateKey(process.env.STRIPE_SECRET_KEY || '', 'secret');
